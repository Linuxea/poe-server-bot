...
async def stream_chat_completion(request: fp.QueryRequest):
    ...
    response = await client.chat.completions.create(
        model="moonshotai/kimi-k2",  # You can change this to any OpenRouter supported model
        messages=messages,
        temperature=1.0,
        stream=True,
        max_tokens=16000,
        extra_headers={
            ...
            "X-Title": "Poe OpenRouter Bot",  # Optional: show in rankings on openrouter.ai
        },
    )
...
image = (
    ...
    .env({"POE_ACCESS_KEY": bot_access_key, "OPENROUTER_API_KEY": openrouter_api_key})
)
